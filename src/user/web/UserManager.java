	...
	public Map<String, Object> login() {
		...
		if (!parameter.get("Text_username").isEmpty() || !parameter.get("Text_password").isEmpty()) {
			if (parameter.get("login_verifycode_name") == null) {
				...
				logger.info("User Login Failed: Username <" + parameter.get("Text_username")+"> with expired verify code.");
			} else {
				...
				if (VerifyCode.checkVerifyCodeAns(parameter.get("login_verifycode_name"),
					...
					// user wrote
					if (checkLogin(parameter.get("Text_username"), parameter.get("Text_password"))) {
						...
						logger.info("User Login Succeeded: Username <" + parameter.get("Text_username") + ">");
					} else {
						result.put("DispatchURL", "index.jsp");
						result.put("Msg", "Username(ID) or password is wrong");
						result.put("Firsttime", false);
						result.put("VerifyCode", VerifyCode.getVerifyCode());
						logger.info("User Login Failed: Username <" + parameter.get("Text_username") +"> with wrong username(ID) or password.");
					}
				else {
					result.put("DispatchURL", "index.jsp");
					...
					result.put("Firsttime", false);
					result.put("VerifyCode", VerifyCode.getVerifyCode());
					logger.info("User Login Failed: Username <" + parameter.get("Text_username")+"> with wrong verify code.");
				}
			}
		} else {
